...
# ================== Trainer Saving Utilities (Step 2) ==================
# 可直接粘贴到 moe_peft/trainer.py 使用
...
def _to_state_dict(weight_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
    """
    规范化 {str: Tensor} 的权重字典，确保均在 CPU 且 contiguous。
    """
    sd = {}
    for k, v in weight_dict.items():
        if not isinstance(v, torch.Tensor):
            raise TypeError(f"Weight value for key {k} is not a Tensor")
        t = v.detach().to("cpu").contiguous()
        sd[str(k)] = t
    return sd


def _save_json(obj, filepath: str):
    """
    保存 JSON（UTF-8、带缩进、兼容中文）。
    """
...
def _save_weight_dict(weight_dict: Dict[str, torch.Tensor], path: str) -> str:
    """
    将 (key->Tensor) 的权重字典保存到目录 `path` 下：
      - 优先保存为 adapter.safetensors（如环境不支持则保存 adapter.pt）
      - 返回实际保存的文件路径
    """
    os.makedirs(path, exist_ok=True)
    state = _to_state_dict(weight_dict)
    if _HAS_SAFETENSORS:
        out_fp = os.path.join(path, "adapter.safetensors")
        _save_safetensors(state, out_fp)
    else:
        out_fp = os.path.join(path, "adapter.pt")
        torch.save(state, out_fp)
    return out_fp


def _dump_config_to_json(cfg_obj) -> dict:
    """
    把 adapter 的配置对象转为可序列化 dict：
    兼容 dict / dataclass / 有 to_dict() / 普通对象 __dict__。
    """
    if cfg_obj is None:
        return {}
    ...
    if hasattr(cfg_obj, "__dict__"):
        ...
        for k, v in cfg_obj.__dict__.items():
            try:
                ...
                out[k] = str(v)
        return out
    return {"_raw": str(cfg_obj)}


def _save_individual_lora(model, config, path: str) -> Dict[str, str]:
    """
    导出当前 adapter（config.adapter_name）的权重与配置到 `path` 目录。
    返回 {"weight": ..., "config": ..., "meta": ...} 的文件路径字典。

    注意：
    - 本函数会调用 model.unload_adapter(adapter_name) 将其从模型卸载。
      若后续仍需使用该 adapter，请再调用 model.init_adapter(lora_config, lora_weight) 重新挂载。
    """
    os.makedirs(path, exist_ok=True)

    adapter_name = getattr(config, "adapter_name", None) or "default"

    # 从模型卸载 adapter 并获取 (lora_config, lora_weight_dict)
    lora_config, lora_weight = model.unload_adapter(adapter_name)

    # 1) 保存权重
    weight_fp = _save_weight_dict(lora_weight, path)

    # 2) 保存配置
    cfg_dict = _dump_config_to_json(lora_config)
    cfg_fp = os.path.join(path, "config.json")
    _save_json(cfg_dict, cfg_fp)

    # 3) 保存元信息
    base_model_name = (
        getattr(getattr(model, "args_", None), "name_or_path_", None)
        or getattr(getattr(model, "config_", None), "name_or_path_", None)
        or getattr(getattr(getattr(model, "model", None), "config", None), "name_or_path", None)
    )
    model_dtype = (
        getattr(model, "dtype", None)
        or getattr(getattr(model, "args_", None), "dtype_", None)
        or str(getattr(model, "parameters", lambda: [])())
    )
    ...
    meta = {
        ...
        "format": "safetensors" if _HAS_SAFETENSORS else "pt",
        "base_model": base_model_name,
        "dtype": model_dtype,
    }
    meta_fp = os.path.join(path, "meta.json")
    _save_json(meta, meta_fp)

    return {"weight": weight_fp, "config": cfg_fp, "meta": meta_fp}


# ====== 可选：读取函数（如需回加载权重/配置，可直接用） ======
...
def _load_weight_dict(path: str) -> Dict[str, torch.Tensor]:
    """
    从目录 `path` 读取 adapter.safetensors 或 adapter.pt，并返回 (key->Tensor) 字典。
    """
    st_path = os.path.join(path, "adapter.safetensors")
    pt_path = os.path.join(path, "adapter.pt")
    if os.path.exists(st_path):
        from safetensors.torch import load_file
        return load_file(st_path)
    if os.path.exists(pt_path):
        return torch.load(pt_path, map_location="cpu")
    raise FileNotFoundError(f"No adapter.safetensors or adapter.pt found under: {path}")
